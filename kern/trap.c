...
void LRU(struct Env *curenv, uint32 fault_va);
void CLOCK(struct Env *curenv, uint32 fault_va);
void FIFO(struct Env *curenv, uint32 fault_va);
...
void page_fault_handler(struct Env * curenv, uint32 fault_va)
{
	//TODO: [FINAL_EVAL_2020 - VER_A] - [2] PAGE FAULT HANDLER
	...
	for(int i = 0; i < curenv->page_WS_max_size; i++)
		if(curenv->ptr_pageWorkingSet[i].empty == 0)
			counter++;
	if (counter < curenv->page_WS_max_size)
		placement(curenv, fault_va);
	//replacement
	else {
		if (isPageReplacmentAlgorithmFIFO())
			FIFO(curenv,fault_va);
		...
		if(ret == E_PAGE_NOT_EXIST_IN_PF){
			if(fault_va >= USTACKBOTTOM && fault_va < USTACKTOP){
				int page = pf_add_empty_env_page(curenv, fault_va, 1);
				if(page == E_NO_PAGE_FILE_SPACE)
					panic("ERROR: No enough virtual space on the page file.");
			}
			else
				panic("ERROR: The faulted address is wrong address.");
		}
...
void placement(struct Env *curenv, uint32 fault_va){
	...
	if(is_page_full){
		...
		env_page_ws_set_time_stamp(curenv,curenv->page_last_WS_index);
	}
}
...
void LRU(struct Env *curenv, uint32 fault_va){
	...
	for (int i = 0; i < curenv->page_WS_max_size; i++){
		if (curenv->ptr_pageWorkingSet[i].time_stamp < mini){
			mini = (unsigned int)curenv->ptr_pageWorkingSet[i].time_stamp;
			changing_index = i;
		}
	}

	uint32 *ptr_page_table = NULL;
	struct Frame_Info *ptr_frame_victim =get_frame_info(curenv->env_page_directory,(void *)curenv->ptr_pageWorkingSet[changing_index].virtual_address, &ptr_page_table);
	int index = PTX(curenv->ptr_pageWorkingSet[changing_index].virtual_address);
	if ((ptr_page_table[index]&PERM_MODIFIED) != 0){
		int ret = pf_update_env_page(curenv, (void *)curenv->ptr_pageWorkingSet[changing_index].virtual_address, ptr_frame_victim);
	}
	unmap_frame(curenv->env_page_directory, (void *)curenv->ptr_pageWorkingSet[changing_index].virtual_address);
	struct Frame_Info *ptr_frame_info = NULL;
	allocate_frame(&ptr_frame_info);
	map_frame(curenv->env_page_directory, ptr_frame_info,(void *)fault_va, PERM_PRESENT|PERM_USER|PERM_WRITEABLE);
	curenv->ptr_pageWorkingSet[changing_index].virtual_address = fault_va;
	env_page_ws_set_time_stamp(curenv,changing_index);
}

void CLOCK(struct Env *curenv, uint32 fault_va){
	while(1){
		uint32* ptr_page_table = NULL;
		get_page_table(curenv->env_page_directory,(void *)curenv->ptr_pageWorkingSet[curenv->page_last_WS_index].virtual_address, &ptr_page_table);
		int index = PTX(curenv->ptr_pageWorkingSet[curenv->page_last_WS_index].virtual_address);
		if ((ptr_page_table[index]&PERM_USED) == 0){
			ptr_page_table[index] = ptr_page_table[index] | PERM_USED;
			struct Frame_Info *ptr_frame_victim =get_frame_info(curenv->env_page_directory,(void *)curenv->ptr_pageWorkingSet[curenv->page_last_WS_index].virtual_address, &ptr_page_table);
			if ((ptr_page_table[index]&PERM_MODIFIED) != 0){
				int ret = pf_update_env_page(curenv, (void *)curenv->ptr_pageWorkingSet[curenv->page_last_WS_index].virtual_address, ptr_frame_victim);
			}
			unmap_frame(curenv->env_page_directory, (void *)curenv->ptr_pageWorkingSet[curenv->page_last_WS_index].virtual_address);
			struct Frame_Info *ptr_frame_info = NULL;
			allocate_frame(&ptr_frame_info);
			map_frame(curenv->env_page_directory, ptr_frame_info,(void *)fault_va, PERM_USED|PERM_PRESENT|PERM_USER|PERM_WRITEABLE);
			curenv->ptr_pageWorkingSet[curenv->page_last_WS_index].virtual_address = fault_va;
			curenv->page_last_WS_index++;
			curenv->page_last_WS_index %= curenv->page_WS_max_size;
			return;
...
void __page_fault_handler_with_buffering(struct Env * curenv, uint32 fault_va)
{
	...
	if(curenv->ptr_pageWorkingSet[curenv->page_last_WS_index].empty == 0)
	{
		...
		while(isFound == 0)
		{
			...
			for(int i=0;i<curenv->page_WS_max_size; i++)
			{
				victim = curenv->ptr_pageWorkingSet[index].virtual_address;
				get_page_table(curenv->env_page_directory, (void*)victim, &ptr_page_table);
				frame_address = ptr_page_table[PTX(victim)];
				uint32 used_bit = frame_address&PERM_USED;
				uint32 modified_bit = frame_address&PERM_MODIFIED;
			...
			for(int i=0;i<curenv->page_WS_max_size; i++)
			{
				...
				uint32 used_bit = frame_address&PERM_USED;
				uint32 modified_bit = frame_address&PERM_MODIFIED;
				if(used_bit == 0 && modified_bit == PERM_MODIFIED){
					curenv->page_last_WS_index = index;
					isFound = 1;
					break;
				}
				ptr_page_table[PTX(victim)] = frame_address&(~PERM_USED);
				index = (index+1)%curenv->page_WS_max_size;
			}
		}
		ptr_page_table[PTX(victim)] &= (~PERM_PRESENT);
		ptr_page_table[PTX(victim)] |= PERM_BUFFERED;
		frame_info = to_frame_info(ptr_page_table[PTX(victim)]);
		frame_info->isBuffered=1;
		frame_info->va = victim;
		frame_info->environment = curenv;

		if((ptr_page_table[PTX(victim)]&PERM_MODIFIED)==PERM_MODIFIED)
			LIST_INSERT_TAIL(&modified_frame_list, frame_info);
		else
			LIST_INSERT_TAIL(&free_frame_list, frame_info);
		if(LIST_SIZE(&modified_frame_list) == getModifiedBufferLength())
		{
			while(modified_frame_list.size > 0)
			{
				frame_info = LIST_FIRST(&modified_frame_list);
				get_page_table(frame_info->environment->env_page_directory,
									(void*)frame_info->va, &ptr_page_table);
				pf_update_env_page(frame_info->environment, (void*)frame_info->va, frame_info);
				ptr_page_table[PTX(frame_info->va)] &= ~(PERM_MODIFIED);
				LIST_REMOVE(&modified_frame_list, frame_info);
				LIST_INSERT_TAIL(&free_frame_list, frame_info);
			}
		}
	}
	get_page_table(curenv->env_page_directory, (void*) fault_va, &ptr_page_table);
	if((ptr_page_table[PTX(fault_va)]&PERM_BUFFERED) == PERM_BUFFERED)
	{
		int frame_num = ptr_page_table[PTX(fault_va)]>>12;
		frames_info[frame_num].isBuffered = 0;

		if((ptr_page_table[PTX(fault_va)]&PERM_MODIFIED) == PERM_MODIFIED)
			LIST_REMOVE(&modified_frame_list, &frames_info[frame_num]);
		else
			LIST_REMOVE(&free_frame_list, &frames_info[frame_num]);
	...
	{
		frame_info = LIST_FIRST(&free_frame_list);
		if(frame_info->isBuffered)
		{
			get_page_table(frame_info->environment->env_page_directory,
					(void*)frame_info->va, &ptr_page_table);
			ptr_page_table[PTX(frame_info->va)] &= ~(PERM_BUFFERED);
			frame_info->isBuffered = 0;
		}

		LIST_REMOVE(&free_frame_list, frame_info);
		map_frame(curenv->env_page_directory, frame_info,
				(void*)fault_va, PERM_USER | PERM_WRITEABLE | PERM_PRESENT);
		int isLoaded = pf_read_env_page(curenv, (void*)fault_va);
		if(isLoaded != 0)
		{
