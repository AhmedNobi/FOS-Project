...
void freeMem(struct Env* e, uint32 virtual_address, uint32 size)
{
	...
	{
		if(e->ptr_pageWorkingSet[i].virtual_address<(virtual_address+size)
				&&e->ptr_pageWorkingSet[i].virtual_address>=virtual_address)
		{
	...
	uint32 x=virtual_address;
	while(x<virtual_address+size)
	{
		uint32 *ptr_page_table=NULL;
		int a=get_page_table(e->env_page_directory,(uint32*)x,&ptr_page_table);
		if(a==0)
		{
			bool flag=0;
			...
			if(!flag)
			{
				uint32 Directory_index= PDX(x);
				uint32 frame_number=e->env_page_directory[Directory_index];
				struct Frame_Info *ptr_frame_info;
				ptr_frame_info=to_frame_info(frame_number);
				ptr_frame_info->references=0;
				free_frame(ptr_frame_info);
				e->env_page_directory[Directory_index]=0;
			}
		}
		x+=PAGE_SIZE;
	}
